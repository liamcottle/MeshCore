...
This document describes the structure of MeshCore packets.
...
- [Header](#header-format): 1 byte
    - bits 0-1: [Route Type](#route-types)
    ...
    - bits 2-5: [Payload Type](#payload-types)
    ...
    - bits 6-7: [Payload Version](#payload-versions)
        - `0x00` - v1 - 1-byte src/dest hashes, 2-byte MAC
        - `0x01` - v2 - Future version (e.g., 2-byte hashes, 4-byte MAC)
        - `0x02` - v3 - Future version
        - `0x03` - v4 - Future version
- Transport Codes: 4 bytes (only present for `ROUTE_TYPE_TRANSPORT_*`)
    - bytes 0 and 1: 16-bit transport code (used for region filtering)
    - bytes 2 and 3: 16-bit transport code
- Path Length: 1 byte (0-64 hops)
- Path: 0-64 bytes (indicated by Path Length)
    - currently contains one byte per path entry
- Payload: 0-184 bytes (remaining data)
